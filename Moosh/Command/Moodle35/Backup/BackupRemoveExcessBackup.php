...
namespace Moosh\Command\Moodle35\Backup;
...
class BackupRemoveExcessBackup extends MooshCommand
{
	...
	require_once($CFG->dirroot . '/backup/util/helper/backup_cron_helper.class.php');
        ...
        if ($all) {
            $courses = null;
            if ($force) {
                echo 'force mode, all courses backups will be cleaned up';
                $courses = get_all_courses();
            } else {
                // Only courses from backup_courses data table
                echo 'normal mode, only courses programmed for backup will be cleaned up';
                $sql = 'SELECT c.*,
	    ...
	    $totalremovedcount = 0;
            foreach($courses as $course){
                ...
                echo 'removed '.($removedcount === false ? 0 : $removedcount).' backup files for course id='.$course->id ;
            }
        } else if (!empty($courseid)) {
            $course = null;
	    echo 'course mode : only course with id='.$courseid.' will be treated';
            if($force){
                echo 'force mode : course backup will be processed anyway';
            ...
            $removedcount = \backup_cron_automated_helper::remove_excess_backups($course, time());
            echo 'remove '.($removedcount === false ? 0 : $removedcount).' backup files for course id='.$course->id ;
        } else {
